...
$navPos = $setting->info['nav_position'];
?>
...
<body>
    ...
    <div id="app">
        ...
        <v-app>
            <!-- Topbar -->
            <v-app-bar app color="<?= $navbarColor; ?>" <?= ($navbarColor == 'white' ? 'light' : 'dark'); ?> :color="$vuetify.theme.dark ? '':'<?= $navbarColor; ?>'" elevation="1">
                <?php if ($navPos == 'top') { ?>
                    <v-toolbar-title class="text-h5 font-weight-medium mr-4" style="cursor: pointer;"><a href="<?= base_url() ?>" class="text-decoration-none <?= ($navbarColor == 'white' ? 'black--text' : 'white--text'); ?>" title="" alt=""><?= env('appName') ?></a></v-toolbar-title>
                <?php } else if ($navPos == 'left') { ?>
                    <v-app-bar-nav-icon @click.stop="sidebarMenu = !sidebarMenu"></v-app-bar-nav-icon>
                    <v-toolbar-title></v-toolbar-title>
                <?php } ?>
                <?php if (session()->get('role') == 1 || session()->get('role') == 2 || session()->get('role') == 3) : ?>
                    <v-btn text class="pa-2 me-2" elevation="0" link href="<?= base_url('sales'); ?>">
                        <v-icon>mdi-cash-register</v-icon>
                    </v-btn>
                <?php endif; ?>
                <?php if (session()->get('role') == 1 || session()->get('role') == 3) : ?>
                    <v-btn text class="pa-2" elevation="0" link href="<?= base_url('pembelian'); ?>">
                ...
                <?php if ($navPos == 'top') : ?>
                    <template v-slot:extension>
                        <v-tabs v-model="tabsMenu" grow center-active show-arrows>
                            <?php if (in_array('menuDashboard', $user_permission)) : ?>
                                <v-tab href="#home" @click="tabHome"><v-icon>mdi-view-dashboard</v-icon>&nbsp;Dashboard</v-tab>
                            <?php endif; ?>

                            <?php if (in_array('menuTransaksi', $user_permission)) : ?>
                                <v-tab href="#transaksi"><v-icon>mdi-swap-horizontal-bold</v-icon>&nbsp;Transaksi</v-tab>
                            <?php endif; ?>

                            <?php if (in_array('menuMaster', $user_permission)) : ?>
                                <v-tab href="#master"><v-icon>mdi-database</v-icon>&nbsp;Master</v-tab>
                            <?php endif; ?>

                            <?php if (in_array('menuLaporan', $user_permission) || in_array('menuStatistik', $user_permission)) : ?>
                                <v-tab href="#report"><v-icon>mdi-file-document</v-icon>&nbsp;Laporan</v-tab>
                            <?php endif; ?>

                            <?php if (in_array('menuSetting', $user_permission)) : ?>
                                <v-tab href="#pengaturan"><v-icon>mdi-store-cog</v-icon>&nbsp;Pengaturan</v-tab>
                            <?php endif; ?>
                        </v-tabs>
                    </template>
                <?php endif; ?>
            ...
            <v-main>
                <?php if ($navPos == 'top') : ?>
                    <v-container fluid>
                        <v-tabs-items v-model="tabsMenu" class="py-1">
                            <v-tab-item value="home">

                            </v-tab-item>

                            <v-tab-item value="transaksi">
                                <?php if (in_array('viewPenjualan', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('penjualan'); ?>" :outlined="buttonMenu == 'penjualan'" :color="buttonMenu == 'penjualan' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-receipt-text</v-icon> Penjualan
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewPembelian', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('pembelian'); ?>" :outlined="buttonMenu == 'pembelian'" :color="buttonMenu == 'pembelian' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-cart</v-icon> Pembelian
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewHutang', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('hutang'); ?>" :outlined="buttonMenu == 'hutang'" :color="buttonMenu == 'hutang' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-tag-arrow-right</v-icon> Hutang
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewPiutang', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('piutang'); ?>" :outlined="buttonMenu == 'piutang'" :color="buttonMenu == 'piutang' ? 'primary':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-book-arrow-left</v-icon> Piutang
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewBiaya', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('biaya'); ?>" :outlined="buttonMenu == 'biaya'" :color="buttonMenu == 'biaya' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-wallet</v-icon> <?= lang('App.cost') ?>
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewCashflow', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('cashflow'); ?>" :outlined="buttonMenu == 'cashflow'" :color="buttonMenu == 'cashflow' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-file-document</v-icon> <?= lang('App.cashflow'); ?>
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewBank', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('bank'); ?>" :outlined="buttonMenu == 'bank'" :color="buttonMenu == 'bank' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-credit-card</v-icon> Bank
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewPajak', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('pajak'); ?>" :outlined="buttonMenu == 'pajak'" :color="buttonMenu == 'pajak' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-file-percent</v-icon> <?= lang('App.tax'); ?>
                                    </v-btn>
                                <?php endif; ?>
                            </v-tab-item>
                            ...
                            <v-tab-item value="master">
                                <?php if (in_array('viewBarang', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('barang'); ?>" :outlined="buttonMenu == 'barang'" :color="buttonMenu == 'barang' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-package-variant-closed</v-icon> Barang
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewStokInOut', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('stok'); ?>" :outlined="buttonMenu == 'stok'" :color="buttonMenu == 'stok' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-package-variant-plus</v-icon> Stok In/Out
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewStokOpname', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('stok_opname'); ?>" :outlined="buttonMenu == 'stok_opname'" :color="buttonMenu == 'stok_opname' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-file-document-edit</v-icon> Stok Opname
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewKontak', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('kontak'); ?>" :outlined="buttonMenu == 'kontak'" :color="buttonMenu == 'kontak' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-account-group</v-icon> <?= lang('App.contact'); ?>
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewUser', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('user'); ?>" :outlined="buttonMenu == 'user'" :color="buttonMenu == 'user' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-account-multiple</v-icon> <?= lang('App.users'); ?>
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewGroup', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('group'); ?>" :outlined="buttonMenu == 'group'" :color="buttonMenu == 'group' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-shield-check</v-icon> Group
                                    </v-btn>
                                <?php endif; ?>
                            </v-tab-item>

                            <v-tab-item value="report">
                                <?php if (in_array('viewLaporan', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('laporan'); ?>" :outlined="buttonMenu == 'laporan'" :color="buttonMenu == 'laporan' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-file-chart</v-icon> <?= lang('App.report') ?>
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewStatistik', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('statistik'); ?>" :outlined="buttonMenu == 'statistik'" :color="buttonMenu == 'statistik' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-chart-bar</v-icon> <?= lang('App.statistic') ?>
                                    </v-btn>
                                <?php endif; ?>
                            </v-tab-item>

                            <v-tab-item value="pengaturan">
                                <?php if (in_array('viewSetting', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('settings'); ?>" :outlined="buttonMenu == 'settings'" :color="buttonMenu == 'settings' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-cog</v-icon> <?= lang('App.application'); ?>
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewConfig', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('toko'); ?>" :outlined="buttonMenu == 'toko'" :color="buttonMenu == 'toko' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-store</v-icon> <?= lang('App.store'); ?>
                                    </v-btn>
                                <?php endif; ?>

                                <?php if (in_array('viewBackup', $user_permission)) : ?>
                                    <v-btn link href="<?= base_url('backup'); ?>" :outlined="buttonMenu == 'backup'" :color="buttonMenu == 'backup' ? 'primary white--text':''" class="mr-2" large elevation="1">
                                        <v-icon>mdi-database</v-icon> Backup DB
                                    </v-btn>
                                <?php endif; ?>
                            </v-tab-item>
                        </v-tabs-items>
                    </v-container>
                    <v-divider v-show="tabsMenu != 'home'"></v-divider>
                <?php endif; ?>
